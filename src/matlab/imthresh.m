function [IDX,sep] = imthresh(I,n)
% Global image imthresh/segmentation function using extended Otsu's method.
%
%   IDX = imthresh(I,N) segments the image I into N classes by means of Otsu's
%   N-level (multi) thresholding method. The function returns an array IDX
%   containing the cluster indices (from 1 to N) of each point.
%   Zero values are assigned to non-finite (NaN or Inf) pixels.
%   If only I is provided then N =2.
%   The number of classes N cannot be more than the image levels (N<255).
%
%   [IDX,sep] = imthresh(...) returns the value (sep) of the separability
...
isRGB = isrgb(I);
assert(isRGB | ndims(I)==2,'The input must be a 2-D array or an RGB image.')
...
P = histo/sum(histo); % regularised histogram
clear unI
...
elseif n==3
    ...
    %options = optimset('PlotFcns','optimplotfval','TolX',1);
